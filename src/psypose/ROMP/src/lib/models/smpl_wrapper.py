...
class SMPLWrapper(nn.Module):
    def __init__(self):
        ...
        self.smpl_model = smpl_model.create(args().smpl_model_path, J_reg_extra9_path=args().smpl_J_reg_extra_path,
                                            J_reg_h36m17_path=args().smpl_J_reg_h37m_path, \
                                            batch_size=args().batch_size, model_type='smpl', gender='neutral',
    ...
    def forward(self, outputs, meta_data):
        ...
        params_dict['body_pose'] = torch.cat(
            [params_dict['body_pose'], torch.zeros(N, 6).to(params_dict['body_pose'].device)], 1)
        ...
        outputs.update({'params': params_dict, 'verts': smpl_outs.vertices, 'j3d': smpl_outs.joints, \
                        'joints_h36m17': smpl_outs.joints_h36m17, 'joints_smpl24': smpl_outs.joints_smpl24,
                        'poses': smpl_outs.full_pose})
