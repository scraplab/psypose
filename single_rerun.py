...
def extract_pose(vpath, render=False):
    ...
    pose.no_render = True
    pose.static_cam = False
    estimate = pose_estimation.estimate_pose(pose)
    return estimate

sessions = screen_paths(data_dir)
sessions = [Path('/safestore/users/landry/SCRAP/data/andromeda_storage/conversations_unconstrained/2023-10-04_000')]
print('##############################\n')
...
if ready != 'y':
    ...
    for s, session in enumerate(sessions):
        ...
        try:
            ...
            #p2 = extract_pose(cam2)
            p1 = extract_pose(cam1)
