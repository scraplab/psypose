...
pose_names = ['cam1_pose_nosmooth.pkl', 'cam2_pose_nosmooth.pkl']
...
def screen_paths(dir):
    ...
    for session in sessions:
        ...
        poses = glob.glob(processed + '/**_pose_nosmooth.pkl')
        poses = [Path(p) for p in poses]
        poses = [p for p in poses if p.exists()]
        if len(poses) != 2:
            to_process.append(session)
        #to_process.append(session) # for now, process all sessions regardless of whether they have pose files
...
def extract_pose(vpath, render=False):
    ...
    pose.no_render = True
    pose.smooth = False  # This is the correct way to disable smoothing
...
for s, session in enumerate(sessions):
    ...
    joblib.dump(p1, outpath / 'cam1_pose_nosmooth.pkl')
    joblib.dump(p2, outpath / 'cam2_pose_nosmooth.pkl')
