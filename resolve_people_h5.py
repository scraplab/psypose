...
def save_people_data(people, filename):
    with h5py.File(filename, 'w') as hf:
        for person_id, data in people.items():
            person_group = hf.create_group(person_id)
            for key, value in data.items():
                if isinstance(value, pd.DataFrame):
                    person_group.create_dataset(key, data=value.to_numpy())
                    person_group[key].attrs['columns'] = value.columns.tolist()
...
def process_session(session):
    ...
    save_people_data(people, session / 'processed' / 'people.h5')
